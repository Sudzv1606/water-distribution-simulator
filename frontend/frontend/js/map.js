...
/**
 * Initialize EPANET network data structure
 */
function initializeEpanetNetworkData() {
    console.log('📊 Initializing EPANET network data...');

    // Define network nodes (EPANET-style components)
    networkNodes = {
        'LAKE': {
            id: 'LAKE',
            name: 'Red Hills Lake',
...
/**
 * Generate realistic acoustic data for pipe
 */
function generateAcousticData(pipe) {
    // Base values influenced by pipe properties
    const baseFreq = 240 + (pipe.diameter / 10); // Larger pipes have lower frequency
    const baseRMS = 0.8 + (pipe.flow / 100); // Higher flow = higher RMS power

    return {
        spectralFreq: Math.round((baseFreq + (Math.random() - 0.5) * 20) * 10) / 10,
        rmsPower: Math.round((Math.max(0.1, Math.min(1.0, baseRMS + (Math.random() - 0.5) * 0.2))) * 1000) / 1000,
        signalQuality: getSignalQuality(pipe)
    };
}
...
/**
 * Position tooltip intelligently to avoid UI blocking
 */
function positionTooltip(tooltip, pipe) {
    ...
    const tooltipWidth = 260; // Reduced width
    ...
    // Viewport boundaries for safety
    const viewportWidth = window.innerWidth;
    const viewportHeight = window.innerHeight;

    // Position tooltip with arrow pointing to pipe
    let tooltipX, tooltipY, positionClass;

    // Prioritize positions that keep tooltip fully visible
    if (spaceRight >= tooltipWidth + 20 && (screenX + tooltipWidth + 20) <= viewportWidth) {
        // Position to the right of pipe
        tooltipX = screenX + 15;
        tooltipY = Math.max(canvasRect.top + 10, Math.min(canvasRect.bottom - tooltipHeight - 10, screenY - tooltipHeight / 2));
        positionClass = 'left';
        ...
        // Position above pipe
        tooltipX = Math.max(canvasRect.left + 10, Math.min(canvasRect.right - tooltipWidth - 10, screenX - tooltipWidth / 2));
        tooltipY = screenY - tooltipHeight - 15;
...
function drawEpanetNode(node) {
    ...
    // Draw flow value for tanks and sources with improved contrast
...
function drawEpanetLabels() {
    ...
    ctx.textAlign = 'center';
    ctx.fillText('Chennai Water Distribution Network (EPANET)', epanetCanvas.width / 2, 30);

    // Draw subtitle
    ctx.fillStyle = '#cbd5e1';
    ctx.font = '14px Inter';
    ctx.fillText('Click on nodes to view comprehensive sensor data • Click pipes to select for scenarios', epanetCanvas.width / 2, 50);
...
function drawEpanetLegend() {
    const legendX = 20;
    const legendY = epanetCanvas.height - 120;
    ...
    ctx.fillText('Network Legend', legendX + 10, legendY + 20);
    ...
    ctx.fillRect(legendX + 10, legendY + 30, 12, 12);
    ctx.fillStyle = '#cbd5e1';
    ctx.fillText('Water Source (Lake/River)', legendX + 25, legendY + 40);

    // Tank
    ctx.fillStyle = '#4ecdc4';
    drawPentagon(legendX + 16, legendY + 52, 6);
    ctx.fillStyle = '#cbd5e1';
    ctx.fillText('Storage Tank', legendX + 25, legendY + 57);

    // Junction
    ctx.fillStyle = '#96ceb4';
    ctx.beginPath();
    ctx.arc(legendX + 16, legendY + 70, 6, 0, 2 * Math.PI);
    ctx.fill();
    ctx.fillStyle = '#cbd5e1';
    ctx.fillText('Junction Node', legendX + 25, legendY + 75);
    ...
    ctx.lineTo(legendX + 20, legendY + 85);
    ctx.stroke();
    ctx.fillText('Normal Pipe', legendX + 25, legendY + 90);
}
...
function injectEpanetLeak(pipeId, severity) {
    console.log('💧 Injecting EPANET leak:', pipeId, severity);
    ...
    if (pipe) {
        ...
        if (startNode) startNode.pressure *= (1 - severity * 0.3);
        if (endNode) endNode.pressure *= (1 - severity * 0.3);
    }
    ...
    fetch('http://localhost:8000/scenarios/leak', {
        ...
        console.error('❌ Error injecting leak:', error);
